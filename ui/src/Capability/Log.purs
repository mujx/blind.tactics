...
-- | the application monad, `Blind.AppM`.
-- |
...
module Blind.Capability.Log where

import Prelude
import Blind.Capability.Now (class Now)
import Blind.Data.LogItem (LogItem, LogReason(..), mkLog)
...
-- | `Log` type from `Blind.Data.Log`. This type can only be constructed using helper functions
...
log :: forall m. Log m => Now m => LogReason -> String -> m Unit
...
logHush :: forall m a. Log m => Now m => LogReason -> m (Either String a) -> m (Maybe a)
